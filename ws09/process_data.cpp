...
namespace sdds
{
	...
	int ProcessData::operator()(const std::string& target_file, double& avg, double& var) {		
		...
		std::vector<double> avgResults(num_threads);
		std::vector<std::thread> varThreads;
		std::vector<double> varResults(num_threads);
		for (int i = 0; i < num_threads; i++) {
			auto thisFunction = std::bind(&computeAvgFactor, data + p_indices[i], p_indices[i + 1] - p_indices[i], total_items, std::ref(avgResults[i]));
			avgThreads.push_back(std::thread(thisFunction));
		}
		...
		for (int i = 0; i < num_threads; i++) {
			auto thisFunction = std::bind(&computeVarFactor, data + p_indices[i], p_indices[i + 1] - p_indices[i], total_items, avg, std::ref(varResults[i]));
			varThreads.push_back(std::thread(thisFunction));
		}
		for (auto& thread : varThreads) {
			thread.join();
		}
		for (double result : varResults) {
			var += result;
		}
		std::ofstream outfile(target_file, std::ios::out | std::ios::binary);
		if (!outfile.is_open()) {
			throw "Unable to open target file";
		}
