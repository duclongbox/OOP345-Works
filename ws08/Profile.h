...
namespace sdds {
	...
	struct Profile {
		...
		bool validateAddress() const {
			...
			if (m_address.postal_code.length() != Address_postal_code_length || !std::isalpha(m_address.postal_code[0]) || !std::isdigit(m_address.postal_code[1]) || !std::isalpha(m_address.postal_code[2]) || m_address.postal_code[3] != ' ' ||!std::isdigit(m_address.postal_code[4]) || !std::isalpha(m_address.postal_code[5]) || !std::isdigit(m_address.postal_code[6])) {
